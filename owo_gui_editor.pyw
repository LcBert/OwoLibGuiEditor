...
class UIComponent:
    def __init__(self, comp_id, comp_type="button", text=""):
        ...
        self.width_method = "content"
        self.width_value = "100"
        self.height_method = "content"
        ...
        self.label_color = ""
        self.label_horiz_text_align = ""
        self.label_vert_text_align = ""
        ...
        # cursor style (button, text-box, slider, checkbox)
        self.cursor_style = ""
        ...
        # entity
        self.entity_id = "minecraft:zombie"
        self.entity_scale = ""
        self.entity_look_at_cursor = "false"

        # item
        self.item_id = "minecraft:stone"

        # texture
        self.texture_location = ""
        self.texture_u = "0"
        self.texture_v = "0"
        self.texture_region_width = "16"
        self.texture_region_height = "16"
...
class UILayout(UIComponent):
    def __init__(self, comp_id, layout_type="Flow Layout", rows="2", cols="2", direction="vertical"):
        ...
        self.flow_gap = ""
        self.surface_flat_color = "#C0101010"
...
class OwoQtDesigner:
    ...
    def create_left_toolbox(self):
        ...
        tk.Button(toolbox, text="+ Slider (slider)", command=lambda: self.add_leaf_node("slider", ""), bg="#444", fg="white").pack(fill=tk.X, padx=15, pady=2)
        tk.Button(toolbox, text="+ Checkbox (checkbox)", command=lambda: self.add_leaf_node("checkbox", ""), bg="#444", fg="white").pack(fill=tk.X, padx=15, pady=2)
        tk.Button(toolbox, text="+ Entità (entity)", command=lambda: self.add_leaf_node("entity", ""), bg="#444", fg="white").pack(fill=tk.X, padx=15, pady=2)
        tk.Button(toolbox, text="+ Item (item)", command=lambda: self.add_leaf_node("item", ""), bg="#444", fg="white").pack(fill=tk.X, padx=15, pady=2)
        tk.Button(toolbox, text="+ Texture (texture)", command=lambda: self.add_leaf_node("texture", ""), bg="#444", fg="white").pack(fill=tk.X, padx=15, pady=2)
    ...
    def create_right_property_panel(self):
        ...
        tk.OptionMenu(
            self.layout_extra_frame, self.prop_l_surface,
            "panel-dark", "panel-light", "vanilla-translucent", "flat", "none",
            command=self.toggle_surface_color_visibility
        ).pack(fill=tk.X)

        # Flat color sub-frame (hidden by default)
        self.surface_flat_frame = tk.Frame(self.layout_extra_frame, bg=self.bg_panel)
        tk.Label(self.surface_flat_frame, text="Colore Flat (hex ARGB):", fg="white", bg=self.bg_panel).pack(anchor=tk.W, pady=2)
        self.prop_surface_flat_color = tk.Entry(self.surface_flat_frame)
        self.prop_surface_flat_color.insert(0, "#C0101010")
        self.prop_surface_flat_color.pack(fill=tk.X, pady=2)
        ...
        # Flow gap (shown only for Flow Layout)
        self.flow_gap_label = tk.Label(self.flow_prop_frame, text="Gap tra figli (gap):", fg="white", bg=self.bg_panel)
        self.prop_flow_gap = tk.Entry(self.flow_prop_frame)
        ...
        # Cursor style (button, text-box, slider, checkbox)
        self.cursor_style_label = tk.Label(self.leaf_extra_frame, text="Cursor Style:", fg="white", bg=self.bg_panel)
        self.prop_cursor_style = tk.StringVar(value="")
        self.cursor_style_menu = tk.OptionMenu(self.leaf_extra_frame, self.prop_cursor_style, "", "hand", "ibeam", "crosshair", "default")

        # Button active
        self.button_active_label = tk.Label(self.leaf_extra_frame, text="Attivo (active):", fg="white", bg=self.bg_panel)
        self.prop_button_active = tk.StringVar(value="")
        self.button_active_menu = tk.OptionMenu(self.leaf_extra_frame, self.prop_button_active, "", "true", "false")
        ...
        # Extended label
        self.label_color_label = tk.Label(self.leaf_extra_frame, text="Colore Testo (color):", fg="white", bg=self.bg_panel)
        self.prop_label_color = tk.Entry(self.leaf_extra_frame)
        self.label_horiz_align_label = tk.Label(self.leaf_extra_frame, text="Allinea Testo H (horiz-text-align):", fg="white", bg=self.bg_panel)
        self.prop_label_horiz_text_align = tk.StringVar(value="")
        self.label_horiz_align_menu = tk.OptionMenu(self.leaf_extra_frame, self.prop_label_horiz_text_align, "", "left", "center", "right")
        self.label_vert_align_label = tk.Label(self.leaf_extra_frame, text="Allinea Testo V (vert-text-align):", fg="white", bg=self.bg_panel)
        self.prop_label_vert_text_align = tk.StringVar(value="")
        self.label_vert_align_menu = tk.OptionMenu(self.leaf_extra_frame, self.prop_label_vert_text_align, "", "top", "center", "bottom")
        self.label_line_spacing_label = tk.Label(self.leaf_extra_frame, text="Interlinea (line-spacing):", fg="white", bg=self.bg_panel)
        self.prop_label_line_spacing = tk.Entry(self.leaf_extra_frame)
        self.label_wrap_width_label = tk.Label(self.leaf_extra_frame, text="Larghezza Wrap (wrap-width):", fg="white", bg=self.bg_panel)
        self.prop_label_wrap_width = tk.Entry(self.leaf_extra_frame)
        ...
        # slider
        self.slider_min_label = tk.Label(self.leaf_extra_frame, text="Minimo (min):", fg="white", bg=self.bg_panel)
        self.prop_slider_min = tk.Entry(self.leaf_extra_frame)
        self.slider_max_label = tk.Label(self.leaf_extra_frame, text="Massimo (max):", fg="white", bg=self.bg_panel)
        self.prop_slider_max = tk.Entry(self.leaf_extra_frame)
        self.slider_step_label = tk.Label(self.leaf_extra_frame, text="Step (step):", fg="white", bg=self.bg_panel)
        self.prop_slider_step = tk.Entry(self.leaf_extra_frame)
        self.slider_value_label = tk.Label(self.leaf_extra_frame, text="Valore (value):", fg="white", bg=self.bg_panel)
        self.prop_slider_value = tk.Entry(self.leaf_extra_frame)

        # checkbox
        self.checkbox_checked_label = tk.Label(self.leaf_extra_frame, text="Selezionato (checked):", fg="white", bg=self.bg_panel)
        self.prop_checkbox_checked = tk.StringVar(value="false")
        self.checkbox_checked_menu = tk.OptionMenu(self.leaf_extra_frame, self.prop_checkbox_checked, "false", "true")

        # entity
        self.entity_id_label = tk.Label(self.leaf_extra_frame, text="Entità ID (entity-id):", fg="white", bg=self.bg_panel)
        self.prop_entity_id = tk.Entry(self.leaf_extra_frame)
        self.entity_scale_label = tk.Label(self.leaf_extra_frame, text="Scala (scale):", fg="white", bg=self.bg_panel)
        self.prop_entity_scale = tk.Entry(self.leaf_extra_frame)
        self.entity_look_label = tk.Label(self.leaf_extra_frame, text="Guarda Cursore (look-at-cursor):", fg="white", bg=self.bg_panel)
        self.prop_entity_look_at_cursor = tk.StringVar(value="false")
        self.entity_look_menu = tk.OptionMenu(self.leaf_extra_frame, self.prop_entity_look_at_cursor, "false", "true")

        # item
        self.item_id_label = tk.Label(self.leaf_extra_frame, text="Item ID (item-id):", fg="white", bg=self.bg_panel)
        self.prop_item_id = tk.Entry(self.leaf_extra_frame)

        # texture
        self.texture_location_label = tk.Label(self.leaf_extra_frame, text="Percorso Texture (texture):", fg="white", bg=self.bg_panel)
        self.prop_texture_location = tk.Entry(self.leaf_extra_frame)
        self.texture_u_label = tk.Label(self.leaf_extra_frame, text="U:", fg="white", bg=self.bg_panel)
        self.prop_texture_u = tk.Entry(self.leaf_extra_frame)
        self.texture_v_label = tk.Label(self.leaf_extra_frame, text="V:", fg="white", bg=self.bg_panel)
        self.prop_texture_v = tk.Entry(self.leaf_extra_frame)
        self.texture_rw_label = tk.Label(self.leaf_extra_frame, text="Larghezza Regione (region-width):", fg="white", bg=self.bg_panel)
        self.prop_texture_region_width = tk.Entry(self.leaf_extra_frame)
        self.texture_rh_label = tk.Label(self.leaf_extra_frame, text="Altezza Regione (region-height):", fg="white", bg=self.bg_panel)
        self.prop_texture_region_height = tk.Entry(self.leaf_extra_frame)
    ...
    def toggle_surface_color_visibility(self, val):
        if val == "flat":
            self.surface_flat_frame.pack(fill=tk.X, pady=2)
        else:
            self.surface_flat_frame.pack_forget()
    ...
    def _hide_leaf_extras_except(self, keep):
        # cursor style: hide only if none of the cursor-capable types are in keep
        cursor_types = {"button", "text-box", "slider", "checkbox"}
        if not cursor_types.intersection(set(keep)):
            self.cursor_style_label.pack_forget(); self.cursor_style_menu.pack_forget()

        if "button" not in keep:
            self.button_active_label.pack_forget(); self.button_active_menu.pack_forget()
        ...
        if "label" not in keep:
            ...
            self.label_color_label.pack_forget(); self.prop_label_color.pack_forget()
            self.label_horiz_align_label.pack_forget(); self.label_horiz_align_menu.pack_forget()
            self.label_vert_align_label.pack_forget(); self.label_vert_align_menu.pack_forget()
            self.label_line_spacing_label.pack_forget(); self.prop_label_line_spacing.pack_forget()
            self.label_wrap_width_label.pack_forget(); self.prop_label_wrap_width.pack_forget()
        ...
        if "slider" not in keep:
            self.slider_min_label.pack_forget(); self.prop_slider_min.pack_forget()
            self.slider_max_label.pack_forget(); self.prop_slider_max.pack_forget()
            self.slider_step_label.pack_forget(); self.prop_slider_step.pack_forget()
            self.slider_value_label.pack_forget(); self.prop_slider_value.pack_forget()

        if "checkbox" not in keep:
            self.checkbox_checked_label.pack_forget(); self.checkbox_checked_menu.pack_forget()

        if "entity" not in keep:
            self.entity_id_label.pack_forget(); self.prop_entity_id.pack_forget()
            self.entity_scale_label.pack_forget(); self.prop_entity_scale.pack_forget()
            self.entity_look_label.pack_forget(); self.entity_look_menu.pack_forget()

        if "item" not in keep:
            self.item_id_label.pack_forget(); self.prop_item_id.pack_forget()

        if "texture" not in keep:
            self.texture_location_label.pack_forget(); self.prop_texture_location.pack_forget()
            self.texture_u_label.pack_forget(); self.prop_texture_u.pack_forget()
            self.texture_v_label.pack_forget(); self.prop_texture_v.pack_forget()
            self.texture_rw_label.pack_forget(); self.prop_texture_region_width.pack_forget()
            self.texture_rh_label.pack_forget(); self.prop_texture_region_height.pack_forget()
    ...
    def render_layout_recursive(self, layout, x1, y1, x2, y2):
        ...
        for i, child in enumerate(layout.children):
            ...
            if isinstance(child, UILayout):
                ...
                colors = {
                    "button": "#3a3a3a", "label": "#151515", "text-box": "#1a2a3a",
                    "text-area": "#1a2a1a", "box": "#2a1a1a", "spacer": "#252525",
                    "slider": "#2a2a3a", "checkbox": "#2a3a2a", "entity": "#1a2a2a",
                    "item": "#2a2a1a", "texture": "#2a1a2a"
                }
                outlines = {
                    "button": "#fff", "label": "#555", "text-box": "#4FC3F7",
                    "text-area": "#81C784", "box": "#FF7043", "spacer": "#888",
                    "slider": "#9C27B0", "checkbox": "#CDDC39", "entity": "#00BCD4",
                    "item": "#FF9800", "texture": "#E91E63"
                }
    ...
    def show_property_editor(self, comp):
        ...
        # Text frame: show for collapsible, and for leaf types except spacer/box/entity/item/texture
        if (isinstance(comp, UILayout) and comp.layout_type == "Collapsible Container") or \
           (not isinstance(comp, UILayout) and comp.type not in ["spacer", "box", "entity", "item", "texture"]):
        ...
        if isinstance(comp, UILayout):
            ...
            # Surface flat color
            self.toggle_surface_color_visibility(comp.surface_type)
            self.prop_surface_flat_color.delete(0, tk.END)
            self.prop_surface_flat_color.insert(0, comp.surface_flat_color)
            ...
            # Hide gap widgets initially
            self.flow_gap_label.pack_forget()
            self.prop_flow_gap.pack_forget()
            ...
            if comp.layout_type in ["Flow Layout", "Scroll Container", "Collapsible Container"]:
                ...
                if comp.layout_type == "Flow Layout":
                    self.flow_gap_label.pack(anchor=tk.W, pady=2)
                    self.prop_flow_gap.pack(fill=tk.X, pady=2)
                    self.prop_flow_gap.delete(0, tk.END)
                    self.prop_flow_gap.insert(0, comp.flow_gap)
            ...
            if comp.type == "label":
                ...
                self.label_color_label.pack(anchor=tk.W); self.prop_label_color.pack(fill=tk.X)
                self.label_horiz_align_label.pack(anchor=tk.W); self.label_horiz_align_menu.pack(fill=tk.X)
                self.label_vert_align_label.pack(anchor=tk.W); self.label_vert_align_menu.pack(fill=tk.X)
                self.label_line_spacing_label.pack(anchor=tk.W); self.prop_label_line_spacing.pack(fill=tk.X)
                self.label_wrap_width_label.pack(anchor=tk.W); self.prop_label_wrap_width.pack(fill=tk.X)
                self.prop_l_shadow.set(comp.label_shadow)
                self.prop_l_mw.delete(0, tk.END); self.prop_l_mw.insert(0, comp.label_max_width)
                self.prop_label_color.delete(0, tk.END); self.prop_label_color.insert(0, comp.label_color)
                self.prop_label_horiz_text_align.set(comp.label_horiz_text_align)
                self.prop_label_vert_text_align.set(comp.label_vert_text_align)
                self.prop_label_line_spacing.delete(0, tk.END); self.prop_label_line_spacing.insert(0, comp.label_line_spacing)
                self.prop_label_wrap_width.delete(0, tk.END); self.prop_label_wrap_width.insert(0, comp.label_wrap_width)

            elif comp.type == "button":
                self._hide_leaf_extras_except(["button"])
                self.cursor_style_label.pack(anchor=tk.W); self.cursor_style_menu.pack(fill=tk.X)
                self.button_active_label.pack(anchor=tk.W); self.button_active_menu.pack(fill=tk.X)
                self.prop_cursor_style.set(comp.cursor_style)
                self.prop_button_active.set(comp.button_active)
                ...
                self._hide_leaf_extras_except(["slider"])
                self.cursor_style_label.pack(anchor=tk.W); self.cursor_style_menu.pack(fill=tk.X)
                self.slider_min_label.pack(anchor=tk.W); self.prop_slider_min.pack(fill=tk.X)
                self.slider_max_label.pack(anchor=tk.W); self.prop_slider_max.pack(fill=tk.X)
                self.slider_step_label.pack(anchor=tk.W); self.prop_slider_step.pack(fill=tk.X)
                self.slider_value_label.pack(anchor=tk.W); self.prop_slider_value.pack(fill=tk.X)
                self.prop_cursor_style.set(comp.cursor_style)
                self.prop_slider_min.delete(0, tk.END); self.prop_slider_min.insert(0, comp.slider_min)
                self.prop_slider_max.delete(0, tk.END); self.prop_slider_max.insert(0, comp.slider_max)
                self.prop_slider_step.delete(0, tk.END); self.prop_slider_step.insert(0, comp.slider_step)
                self.prop_slider_value.delete(0, tk.END); self.prop_slider_value.insert(0, comp.slider_value)

            elif comp.type == "checkbox":
                self._hide_leaf_extras_except(["checkbox"])
                self.cursor_style_label.pack(anchor=tk.W); self.cursor_style_menu.pack(fill=tk.X)
                self.checkbox_checked_label.pack(anchor=tk.W); self.checkbox_checked_menu.pack(fill=tk.X)
                self.prop_cursor_style.set(comp.cursor_style)
                self.prop_checkbox_checked.set(comp.checkbox_checked)

            elif comp.type == "entity":
                self._hide_leaf_extras_except(["entity"])
                self.entity_id_label.pack(anchor=tk.W); self.prop_entity_id.pack(fill=tk.X)
                self.entity_scale_label.pack(anchor=tk.W); self.prop_entity_scale.pack(fill=tk.X)
                self.entity_look_label.pack(anchor=tk.W); self.entity_look_menu.pack(fill=tk.X)
                self.prop_entity_id.delete(0, tk.END); self.prop_entity_id.insert(0, comp.entity_id)
                self.prop_entity_scale.delete(0, tk.END); self.prop_entity_scale.insert(0, comp.entity_scale)
                self.prop_entity_look_at_cursor.set(comp.entity_look_at_cursor)

            elif comp.type == "item":
                self._hide_leaf_extras_except(["item"])
                self.item_id_label.pack(anchor=tk.W); self.prop_item_id.pack(fill=tk.X)
                self.prop_item_id.delete(0, tk.END); self.prop_item_id.insert(0, comp.item_id)

            elif comp.type == "texture":
                self._hide_leaf_extras_except(["texture"])
                self.texture_location_label.pack(anchor=tk.W); self.prop_texture_location.pack(fill=tk.X)
                self.texture_u_label.pack(anchor=tk.W); self.prop_texture_u.pack(fill=tk.X)
                self.texture_v_label.pack(anchor=tk.W); self.prop_texture_v.pack(fill=tk.X)
                self.texture_rw_label.pack(anchor=tk.W); self.prop_texture_region_width.pack(fill=tk.X)
                self.texture_rh_label.pack(anchor=tk.W); self.prop_texture_region_height.pack(fill=tk.X)
                self.prop_texture_location.delete(0, tk.END); self.prop_texture_location.insert(0, comp.texture_location)
                self.prop_texture_u.delete(0, tk.END); self.prop_texture_u.insert(0, comp.texture_u)
                self.prop_texture_v.delete(0, tk.END); self.prop_texture_v.insert(0, comp.texture_v)
                self.prop_texture_region_width.delete(0, tk.END); self.prop_texture_region_width.insert(0, comp.texture_region_width)
                self.prop_texture_region_height.delete(0, tk.END); self.prop_texture_region_height.insert(0, comp.texture_region_height)
    ...
    def apply_properties(self):
        ...
        if not isinstance(self.selected_component, UILayout) and self.selected_component.type not in ["spacer", "box", "entity", "item", "texture"]:
        ...
        if isinstance(self.selected_component, UILayout):
            ...
            self.selected_component.surface_flat_color = self.prop_surface_flat_color.get()

            if self.selected_component.layout_type in ["Flow Layout", "Scroll Container", "Collapsible Container"]:
                self.selected_component.direction = self.prop_l_direction.get()
            if self.selected_component.layout_type == "Flow Layout":
                self.selected_component.flow_gap = self.prop_flow_gap.get()
            ...
            # cursor style for applicable types
            if comp.type in ["button", "text-box", "slider", "checkbox"]:
                comp.cursor_style = self.prop_cursor_style.get()

            if comp.type == "label":
                comp.label_shadow = self.prop_l_shadow.get()
                comp.label_max_width = self.prop_l_mw.get()
                comp.label_color = self.prop_label_color.get()
                comp.label_horiz_text_align = self.prop_label_horiz_text_align.get()
                comp.label_vert_text_align = self.prop_label_vert_text_align.get()
                comp.label_line_spacing = self.prop_label_line_spacing.get()
                comp.label_wrap_width = self.prop_label_wrap_width.get()
            elif comp.type == "button":
                comp.button_active = self.prop_button_active.get()
            elif comp.type == "text-box":
                comp.text_box_max_length = self.prop_tb_max_length.get()
            elif comp.type == "text-area":
                comp.text_area_max_length = self.prop_ta_max_length.get()
                comp.text_area_max_lines = self.prop_ta_max_lines.get()
                comp.text_area_display_char_count = self.prop_ta_display_count.get()
            elif comp.type == "box":
                comp.box_color_mode = self.prop_box_color_mode.get()
                comp.box_color = self.prop_box_color.get()
                comp.box_start_color = self.prop_box_start_color.get()
                comp.box_end_color = self.prop_box_end_color.get()
                comp.box_fill = self.prop_box_fill.get()
                comp.box_direction = self.prop_box_direction.get()
            elif comp.type == "spacer":
                comp.spacer_percent = self.prop_spacer_percent.get()
            elif comp.type == "slider":
                comp.slider_min = self.prop_slider_min.get()
                comp.slider_max = self.prop_slider_max.get()
                comp.slider_step = self.prop_slider_step.get()
                comp.slider_value = self.prop_slider_value.get()
            elif comp.type == "checkbox":
                comp.checkbox_checked = self.prop_checkbox_checked.get()
            elif comp.type == "entity":
                comp.entity_id = self.prop_entity_id.get()
                comp.entity_scale = self.prop_entity_scale.get()
                comp.entity_look_at_cursor = self.prop_entity_look_at_cursor.get()
            elif comp.type == "item":
                comp.item_id = self.prop_item_id.get()
            elif comp.type == "texture":
                comp.texture_location = self.prop_texture_location.get()
                comp.texture_u = self.prop_texture_u.get()
                comp.texture_v = self.prop_texture_v.get()
                comp.texture_region_width = self.prop_texture_region_width.get()
                comp.texture_region_height = self.prop_texture_region_height.get()
...
# =====================================================================
    ...
    def serialize_node_recursive(self, xml_parent, obj, grid_pos=None):
        ...
        # flow-layout gap
        if isinstance(obj, UILayout) and obj.layout_type == "Flow Layout" and obj.flow_gap:
            ET.SubElement(node, "gap").text = obj.flow_gap

        # button: active before text/sizing
        if tag == "button":
            if obj.button_active in ("true", "false"):
                ET.SubElement(node, "active").text = obj.button_active

        # text for button, label, collapsible, text-box (not checkbox — handled separately)
        if tag in ["button", "label", "collapsible", "text-box"] and obj.text:
            ET.SubElement(node, "text").text = obj.text

        # cursor-style for applicable types
        if tag in ["button", "text-box", "slider", "checkbox"] and obj.cursor_style:
            ET.SubElement(node, "cursor-style").text = obj.cursor_style
        ...
        if tag == "label":
            ...
            if obj.label_color: ET.SubElement(node, "color").text = obj.label_color
            if obj.label_horiz_text_align: ET.SubElement(node, "horizontal-text-alignment").text = obj.label_horiz_text_align
            if obj.label_vert_text_align: ET.SubElement(node, "vertical-text-alignment").text = obj.label_vert_text_align
            if obj.label_line_spacing: ET.SubElement(node, "line-spacing").text = obj.label_line_spacing
            if obj.label_wrap_width: ET.SubElement(node, "wrap-width").text = obj.label_wrap_width

        if tag == "slider":
            ET.SubElement(node, "min").text = obj.slider_min
            ET.SubElement(node, "max").text = obj.slider_max
            if obj.slider_step: ET.SubElement(node, "step").text = obj.slider_step
            if obj.slider_value: ET.SubElement(node, "value").text = obj.slider_value

        if tag == "checkbox":
            if obj.text: ET.SubElement(node, "text").text = obj.text
            ET.SubElement(node, "checked").text = obj.checkbox_checked

        if tag == "entity":
            if obj.entity_id: ET.SubElement(node, "entity-id").text = obj.entity_id
            if obj.entity_scale: ET.SubElement(node, "scale").text = obj.entity_scale
            if obj.entity_look_at_cursor == "true": ET.SubElement(node, "look-at-cursor").text = "true"

        if tag == "item":
            if obj.item_id: ET.SubElement(node, "item-id").text = obj.item_id

        if tag == "texture":
            if obj.texture_location: ET.SubElement(node, "texture").text = obj.texture_location
            ET.SubElement(node, "u").text = obj.texture_u
            ET.SubElement(node, "v").text = obj.texture_v
            ET.SubElement(node, "region-width").text = obj.texture_region_width
            ET.SubElement(node, "region-height").text = obj.texture_region_height
        ...
        if isinstance(obj, UILayout):
            ...
            if obj.surface_type != "none":
                ...
                if obj.surface_type == "panel-dark":
                    ET.SubElement(surface_node, "panel", {"dark": "true"})
                elif obj.surface_type == "panel-light":
                    ET.SubElement(surface_node, "panel", {"dark": "false"})
                elif obj.surface_type == "vanilla-translucent":
                    ET.SubElement(surface_node, "vanilla-translucent")
                elif obj.surface_type == "flat":
                    flat_node = ET.SubElement(surface_node, "flat")
                    flat_node.text = obj.surface_flat_color
    ...
    def import_xml(self):
        ...
        try:
            ...
            if components is not None:
                main_flow = (components.find("flow-layout") or components.find("grid-layout") or
                             components.find("stack-layout") or components.find("scroll") or
                             components.find("drag") or components.find("collapsible") or
                             components.find("dropdown"))
            else:
                templates = root.find("templates")
                if templates is not None:
                    template_el = templates.find("template")
                    if template_el is not None:
                        main_flow = (template_el.find("flow-layout") or template_el.find("grid-layout") or
                                     template_el.find("stack-layout") or template_el.find("scroll") or
                                     template_el.find("drag") or template_el.find("collapsible") or
                                     template_el.find("dropdown"))
                        is_template = True
                        t_name = template_el.get("name", "custom_template")

            if main_flow is None:
                messagebox.showerror("Errore", "Impossibile trovare un layout radice valido (<flow-layout>, <grid-layout>, <stack-layout> o <scroll/drag/collapsible/dropdown>)")
                return
            ...
            if main_flow.tag == "grid-layout":
                ...
                l_type = "Flow Layout"
            elif main_flow.tag == "stack-layout":
                l_type = "Stack Layout"
            ...
            # gap for root flow-layout
            if l_type == "Flow Layout":
                gap_n = main_flow.find("gap")
                if gap_n is not None: self.main_layout.flow_gap = safe_text(gap_n, "")

            surf_node = main_flow.find("surface")
            if surf_node is not None:
                panel_el = surf_node.find("panel")
                if panel_el is not None:
                    dark_attr = panel_el.get("dark", "true")
                    self.main_layout.surface_type = "panel-dark" if dark_attr != "false" else "panel-light"
                elif surf_node.find("vanilla-translucent") is not None:
                    self.main_layout.surface_type = "vanilla-translucent"
                elif surf_node.find("flat") is not None:
                    self.main_layout.surface_type = "flat"
                    self.main_layout.surface_flat_color = safe_text(surf_node.find("flat"), "#C0101010")
                else:
            ...
            if l_type in ["Scroll Container", "Draggable Container"]:
                for sub in main_flow:
                    if sub.tag not in ["sizing", "margins", "padding", "surface", "text", "horizontal-alignment", "vertical-alignment", "gap"]:
    ...
    def parse_xml_recursive(self, xml_children, parent_obj):
        for child in xml_children:
            if child.tag in ["flow-layout", "grid-layout", "stack-layout", "scroll", "drag", "collapsible", "dropdown"]:
                ...
                # gap for flow-layout
                if l_type == "Flow Layout":
                    gap_n = child.find("gap")
                    if gap_n is not None: sub_layout.flow_gap = safe_text(gap_n, "")
                ...
                if surf_node is not None:
                    ...
                    if panel_el is not None:
                        dark_attr = panel_el.get("dark", "true")
                        sub_layout.surface_type = "panel-dark" if dark_attr != "false" else "panel-light"
                    elif surf_node.find("vanilla-translucent") is not None:
                        sub_layout.surface_type = "vanilla-translucent"
                    elif surf_node.find("flat") is not None:
                        sub_layout.surface_type = "flat"
                        sub_layout.surface_flat_color = safe_text(surf_node.find("flat"), "#C0101010")
                    else:
                ...
                if l_type in ["Scroll Container", "Draggable Container"]:
                    for sub in child:
                        if sub.tag not in ["sizing", "margins", "padding", "surface", "text", "horizontal-alignment", "vertical-alignment", "children", "gap"]:
                    ...
                    if inner_children is not None: self.parse_xml_recursive(inner_children, sub_layout)
                                ...
                                "slider", "checkbox", "entity", "item", "texture"]:
                ...
                # cursor-style for all applicable types
                cs = child.find("cursor-style")
                if cs is not None: leaf_node.cursor_style = safe_text(cs, "")

                if child.tag == "button":
                    act = child.find("active")
                    if act is not None: leaf_node.button_active = safe_text(act, "")
                ...
                if child.tag == "label":
                    ...
                    color_n = child.find("color")
                    if color_n is not None: leaf_node.label_color = safe_text(color_n, "")
                    hta = child.find("horizontal-text-alignment")
                    if hta is not None: leaf_node.label_horiz_text_align = safe_text(hta, "")
                    vta = child.find("vertical-text-alignment")
                    if vta is not None: leaf_node.label_vert_text_align = safe_text(vta, "")
                    ls = child.find("line-spacing")
                    if ls is not None: leaf_node.label_line_spacing = safe_text(ls, "")
                    ww = child.find("wrap-width")
                    if ww is not None: leaf_node.label_wrap_width = safe_text(ww, "")
                ...
                if child.tag == "slider":
                    mn = child.find("min");
                    if mn is not None: leaf_node.slider_min = safe_text(mn, "0")
                    mx = child.find("max");
                    if mx is not None: leaf_node.slider_max = safe_text(mx, "100")
                    st = child.find("step");
                    if st is not None: leaf_node.slider_step = safe_text(st, "")
                    vl = child.find("value");
                    if vl is not None: leaf_node.slider_value = safe_text(vl, "0")

                if child.tag == "checkbox":
                    ck = child.find("checked");
                    if ck is not None: leaf_node.checkbox_checked = safe_text(ck, "false")

                if child.tag == "entity":
                    ei = child.find("entity-id");
                    if ei is not None: leaf_node.entity_id = safe_text(ei, "minecraft:zombie")
                    sc = child.find("scale");
                    if sc is not None: leaf_node.entity_scale = safe_text(sc, "")
                    lac = child.find("look-at-cursor");
                    if lac is not None: leaf_node.entity_look_at_cursor = safe_text(lac, "false")

                if child.tag == "item":
                    ii = child.find("item-id");
                    if ii is not None: leaf_node.item_id = safe_text(ii, "minecraft:stone")

                if child.tag == "texture":
                    tl = child.find("texture");
                    if tl is not None: leaf_node.texture_location = safe_text(tl, "")
                    u = child.find("u");
                    if u is not None: leaf_node.texture_u = safe_text(u, "0")
                    v = child.find("v");
                    if v is not None: leaf_node.texture_v = safe_text(v, "0")
                    rw = child.find("region-width");
                    if rw is not None: leaf_node.texture_region_width = safe_text(rw, "16")
                    rh = child.find("region-height");
                    if rh is not None: leaf_node.texture_region_height = safe_text(rh, "16")
